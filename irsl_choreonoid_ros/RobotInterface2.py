...
class JointGroupTopic(JointGroupBase):
    def __init__(self, group, name, robot=None):
        ...
        self.finish_time = TODO_FIX_ROS1_ROSPY.get_rostime()
    ...
    def sendAngles(self, tm = None):  ## override
        ...
        point.time_from_start = TODO_FIX_ROS1_ROSPY.Duration(tm)
        msg.points.append(point)
        self.finish_time = TODO_FIX_ROS1_ROSPY.get_rostime() + TODO_FIX_ROS1_ROSPY.Duration(tm)
        self.pub.publish(msg)
    ...
    def sendAnglesSequence(self, vec_list, tm_list):  ## override
        ...
        for vec, tm in zip(vec_list, tm_list):
            ...
            point.positions = vec
            time_ += tm
            point.time_from_start = TODO_FIX_ROS1_ROSPY.Duration(time_)
            msg.points.append(point)
        self.finish_time = TODO_FIX_ROS1_ROSPY.get_rostime() + TODO_FIX_ROS1_ROSPY.Duration(time_)
        self.pub.publish(msg)

    def isFinished(self):  ## override
        diff = (TODO_FIX_ROS1_ROSPY.get_rostime() - self.finish_time).to_sec()
        if diff > 0:
            return True
        else:
            return False
    ...
    def waitUntilFinish(self, timeout=None):  ## override
        ...
        st = TODO_FIX_ROS1_ROSPY.get_rostime()
        while (TODO_FIX_ROS1_ROSPY.get_rostime() - st).to_sec() < timeout:
            if self.isFinished():
                break
...
class JointGroupAction(JointGroupBase):
    ...
    def waitUntilFinish(self, timeout=None):  ## override
        ## TODO update for ROS2
        if timeout is None:
            timeout = TODO_FIX_ROS1_ROSPY.Duration(1000000000.0)
        else:
            if timeout <= 0.0:
                timeout = 0.001
            timeout = TODO_FIX_ROS1_ROSPY.Duration(timeout)
...
class RobotInterface(Node, JointInterface, DeviceInterface, MobileBaseInterface):
    ...
    """
#>    def ROS1__init__(self, file_name, node_name='robot_interface', anonymous=False, connection_wait=3.0, connection=True,
...
#>        ## wait connected
#>        tmp = TODO_FIX_ROS1_ROSPY.get_rostime()
#>        connect_=False
#>        while (TODO_FIX_ROS1_ROSPY.get_rostime() - tmp).to_sec() < connection_wait:
#>            res = True
...
#>        self.__connection=connect_
    ...
    def __load_robot(self):
        if 'robot_model' in self.info:
            ...
            ##TODO_FIX_ROS1_ROSPY.loginfo('loading model from {}'.format(self.model_file))
    ...
    def isRealRobot(self):
        ...
        """
        res_ = TODO_FIX_ROS1_ROSPY.has_param('/use_sim_time')
        if res_:
            res_ = TODO_FIX_ROS1_ROSPY.get_param('/use_sim_time')
        return not res_
